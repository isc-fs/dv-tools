...
ISC MingoROS. The project's biggest feasibility risk — can a pure-Rust DDS client
...
- **Client:** the `mingoros` CLI built with `--features ros2`, packaged as a
  ...
  so a macOS-native client cannot see the graph. **ISC MingoROS runs *inside* the
...
Discovery output (excerpt) — note ISC MingoROS's own `dv_contract` QoS annotations
