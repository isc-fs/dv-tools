//! MingoROS Studio — the native desktop dashboard (Tauri).
//!
//! Same `mingoros-core` engine as the CLI, in a window. The app joins the
//! car's ROS 2 DDS graph directly over Ethernet (the `ros2`/RustDDS backend)
//! and serves the shared `apps/mingoros-studio/ui` frontend, which calls the
//! commands below via `window.__TAURI__.core.invoke`.
