# Connecting ISC MingoROS to the DV PC (direct RJ45 cable)

ISC MingoROS joins the car's ROS 2 DDS graph directly — no ROS install on the laptop
...
> Once ISC MingoROS is on the graph it can command actuation (EBS, control/mission
> topics) and a mistaken command can move the car. Keep it jacked up and
> freewheeling the entire time it's connected.

## The one thing to know first

RustDDS (the DDS stack ISC MingoROS uses) discovers peers over **UDP multicast**
...
**multicast reach across the cable**. ISC MingoROS's lever is the **interface bind**:
...
### 3. Point ISC MingoROS at the cable
...
- **A switch instead of a direct cable** works too, as long as it forwards
  ...
  subnet.
- **macOS + Docker Desktop:** a native Mac ISC MingoROS **cannot** see a pipeline
